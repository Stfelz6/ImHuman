...
export default function CampaignManager(props) {
  ...
  const makeInactive = async () => {
    ...
    try {
      ...
      const updatedCampaignDataArray = props.campaignDataArray.map((campaign) =>
        campaign.id === updatedCampaign.id ? updatedCampaign : campaign
      );
      props.setCampaignDataArray(updatedCampaignDataArray);

      
      // Update filteredCampaigns by removing the updated campaign
      const updatedfilteredCampaigns = props.campaignDataArray.filter(
        (campaign) => campaign.id !== props.campaignData.id
      );
      props.setFilteredCampaigns(updatedfilteredCampaigns);

      props.setActiveFilter('Pending');
  
      props.setAlert({
        type: 'inactiveCampaign',
        title: 'Campaign inactive!',
        body: 'The campaign will now be added to the Pending category.',
      });
    } catch (error) {
  ...
  const makeActive = async () => {
    ...
    try {
      ...
      props.setActiveFilter('Active');

      props.setAlert({type:'approveCampaign',title:'Campaign approved!', body:'The campaign will now be seen on the main page.'});
  ...
  const deteleCampaign = async () => {
    ...
    try {
      ...
      console.log("Campaign has been deleted:", updateCampaign);
  
      // Update campaignDataArray by removing the deleted campaign
      const updatedCampaignDataArray = props.campaignDataArray.filter(
        (campaign) => campaign.id !== props.campaignData.id
      );
      props.setCampaignDataArray(updatedCampaignDataArray);

      // Update filteredCampaigns by removing the deleted campaign
      const updatedfilteredCampaigns = props.campaignDataArray.filter(
        (campaign) => campaign.id !== props.campaignData.id
      );
      props.setFilteredCampaigns(updatedfilteredCampaigns);
